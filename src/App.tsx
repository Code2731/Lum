...
import { Copy, Zap, Minus, Square, X, Trash2, Settings, Plus, Search, Columns, Rows, CheckCircle2, Play, FilePlus, Download, HardDrive, Loader2, History, File, Layers, BrainCircuit, Monitor, Globe, RefreshCcw } from "lucide-react";
...
interface Action {
  type: "run" | "create";
  cmd?: string; path?: string; content?: string; label: string;
  status?: "pending" | "running" | "completed" | "error";
}

interface TerminalBlock {
  id: string; command: string; output: string; explanation?: string; actions?: Action[]; analysis?: string; suggestion?: string; type: "shell" | "ai" | "error-analysis"; status: "executing" | "completed" | "error"; cwd: string; gitBranch: string | null; embedding?: number[];
}

interface AppConfig {
  theme: string; font_size: number; opacity: number; accent_color: string;
}

interface Pane {
  id: string; blocks: TerminalBlock[];
}

interface Tab {
  id: string; name: string; panes: Pane[]; activePaneId: string; orientation: "horizontal" | "vertical";
}

interface PullProgress { status: string; total?: number; completed?: number; }
interface IndexProgress { current: number; total: number; }
interface CodeChunk { path: string; content: string; }
interface PaletteItem { id: string; type: "file" | "command" | "tab" | "action"; title: string; subtitle?: string; icon: React.ReactNode; onSelect: () => void; }
...
const App = () => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      else if (isCmd && e.key === "b") { e.preventDefault(); setShowWebview(prev => !prev); }
  ...
  useEffect(() => {
    if (tabs.length > 0) {
      const toSave = tabs.map(t => ({
        ...t, panes: t.panes.map(p => ({ ...p, blocks: p.blocks.slice(-50).map(b => ({ ...b, status: b.status === "executing" ? "completed" : b.status })) }))
      }));
    ...
    const init = async () => {
      try {
        const [savedTabs, savedConfig] = await Promise.all([
          invoke<Tab[]>("load_session"), invoke<AppConfig>("load_config")
        ]);
        if (savedConfig) { setConfig(savedConfig); applyTheme(savedConfig); }
        const ctx = await invoke<{ cwd: string; git_branch: string | null; files: string[]; project_summary: string }>("get_system_context");
        setContext(ctx);
        if (savedTabs && savedTabs.length > 0) {
          setTabs(savedTabs); setActiveTabId(savedTabs[0].id);
          for (const tab of savedTabs) { for (const pane of tab.panes) { await invoke("spawn_pty", { tabId: pane.id, cwd: pane.blocks.slice(-1)[0]?.cwd }).catch(() => {}); } }
        } else createTab("Terminal 1");
      } catch (e) { createTab("Terminal 1"); }
    };
    init();
  ...
  useEffect(() => { syncOllama(); const iv = setInterval(syncOllama, 10000); return () => clearInterval(iv); }, [syncOllama]);

  useEffect(() => {
    const unlistenPty = listen<{ tab_id: string; data: string }>("pty-data", (event) => {
      const { tab_id, data } = event.payload;
      setTabs((prev) => prev.map((t) => ({ ...t, panes: t.panes.map((p) => p.id === tab_id ? { ...p, blocks: p.blocks.map((b, i, arr) => i === arr.length - 1 && b.status === "executing" ? { ...b, output: b.output + data } : b), } : p) })));
    });
    const unlistenPull = listen<PullProgress>("pull-progress", (event) => {
      setPullProgress(event.payload); if (event.payload.status === "success") { syncOllama(); setPullProgress(null); }
    });
    const unlistenIndex = listen<IndexProgress>("index-progress", (event) => { setIndexProgress(event.payload); });
    return () => { unlistenPty.then(f => f()); unlistenPull.then(f => f()); unlistenIndex.then(f => f()); };
  }, [syncOllama]);
  ...
  const handleIndexProject = async () => {
    ...
    } catch (e) { alert(`Indexing failed: ${e}`); }
    finally { setIsIndexing(false); setIndexProgress(null); }
  };
  ...
  const handleCommand = useCallback(async (cmd: string, type: "shell" | "ai") => {
    ...
    try {
      if (type === "ai") {
        const searchResults = await invoke<CodeChunk[]>("search_codebase", { query: cmd, model: selectedModel }).catch(() => []);
        const codebaseContext = searchResults.map(r => `File: ${r.path}\nContent:\n${r.content}`).join("\n---\n");
        const fullContext = `Project Summary:\n${context.project_summary}\n\nRelevant Code:\n${codebaseContext}\n\nCWD: ${context.cwd}${showWebview ? `\nWatching: ${webviewUrl}` : ""}`;
        const result = await invoke<string>("generate_ai_command", { prompt: cmd, model: selectedModel, context: fullContext });
        const jsonMatch = result.match(/\{[\s\S]*\}/);
        const parsed = jsonMatch ? JSON.parse(jsonMatch[0]) : null;
        setTabs(prev => prev.map(t => t.id === activeTab.id ? { ...t, panes: t.panes.map(p => p.id === paneId ? { ...p, blocks: p.blocks.map(b => b.id === id ? { ...b, command: parsed?.command || b.command, explanation: parsed?.explanation || "", actions: parsed?.actions || [], output: parsed ? `$ ${parsed.command}` : result, status: "completed" as const } : b) } : p) } : t));
  ...
  const updateConfig = async (newCfg: Partial<AppConfig>) => {
    const updated = { ...config, ...newCfg }; setConfig(updated); applyTheme(updated);
    await invoke("save_config", { config: updated });
  };

  useEffect(() => {
    if (isPaletteOpen) { setTimeout(() => paletteInputRef.current?.focus(), 10); setPaletteSearch(""); }
  }, [isPaletteOpen]);
  ...
  useEffect(() => {
    ...
    const buildItems = async () => {
      ...
      tabs.forEach((t, i) => items.push({ id: `tab-${t.id}`, type: "tab", title: `Switch to: ${t.name}`, subtitle: `Tab ${i+1}`, icon: <Layers size={14}/>, onSelect: () => setActiveTabId(t.id) }));
      
      const history = Array.from(new Set(tabs.flatMap(t => t.panes.flatMap(p => p.blocks.map(b => b.command))))).slice(-20);
      history.forEach((cmd, i) => items.push({ id: `hist-${i}`, type: "command", title: cmd, subtitle: "History", icon: <History size={14}/>, onSelect: () => handleCommand(cmd, "shell") }));
      
      try {
        const files = await invoke<string[]>("get_project_files");
        files.slice(0, 100).forEach(f => items.push({ id: `file-${f}`, type: "file", title: f.split("/").pop() || f, subtitle: f, icon: <File size={14}/>, onSelect: () => handleCommand(`cat ${f}`, "shell") }));
      } catch {}
        ...
        if (query.length > 2) {
          try {
            ...
            setPaletteResults(scored.map(s => ({
              id: `semantic-${s.block.id}`, type: "command", title: s.block.command, subtitle: `유사도: ${(s.score * 100).toFixed(1)}% | ${s.block.output.slice(0, 40).replace(/\n/g, " ")}`, icon: <Zap size={14}/>, onSelect: () => handleCommand(s.block.command, "shell")
            })));
            setPaletteIdx(0);
            return;
          } catch {}
      ...
      if (!paletteSearch) { setPaletteResults(items.slice(0, 10)); return; }
    ...
    buildItems();
  }, [isPaletteOpen, paletteSearch, tabs, createTab, splitPane, selectedModel, ollamaOnline, handleCommand]);

  const handlePaletteKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === "ArrowDown") { e.preventDefault(); setPaletteIdx(prev => (prev + 1) % (paletteResults.length || 1)); }
    else if (e.key === "ArrowUp") { e.preventDefault(); setPaletteIdx(prev => (prev - 1 + paletteResults.length) % (paletteResults.length || 1)); }
  ...
  const handleRunAction = async (blockId: string, actionIdx: number) => {
    if (!activeTab || !activeTab.activePaneId) return;
    const paneId = activeTab.activePaneId;
    const action = activePane?.blocks.find(b => b.id === blockId)?.actions?.[actionIdx];
    if (!action) return;
  ...
  const handleAutoFix = async (block: TerminalBlock) => {
    ...
    try {
      ...
      if (parsed.suggestion) setTimeout(() => handleCommand(parsed.suggestion, "shell"), 1500);
    } catch {
    ...
    <div className="app-root">
            ...
            <div className="palette-results">
              {paletteResults.map((item, idx) => (
                <div key={item.id} className={`palette-item ${idx === paletteIdx ? "active" : ""}`} onClick={() => { item.onSelect(); setIsPaletteOpen(false); }}>
                  ...
                  <div className="palette-item-info"><div className="palette-item-title">{item.title}</div>{item.subtitle && <small className="palette-item-subtitle">{item.subtitle}</small>}</div>
      ...
      <header className="titlebar" onMouseDown={(e) => { if (!(e.target as HTMLElement).closest("button, select, input")) appWindow.startDragging(); }}>
        ...
        <div className="titlebar-controls">
          <button className={`titlebar-btn ${showWebview ? "active" : ""}`} onClick={() => setShowWebview(prev => !prev)} title="Toggle Visual Browser (Cmd+B)"><Monitor size={14} /></button>
          <button className="titlebar-btn" onClick={() => splitPane("vertical")} title="Split Vertical"><Columns size={14} /></button>
          <button className="titlebar-btn" onClick={() => splitPane("horizontal")} title="Split Horizontal"><Rows size={14} /></button>
          <button className="titlebar-btn" onClick={() => setIsSettingsOpen(true)} title="Settings"><Settings size={14} /></button>
          <button className="titlebar-btn" onClick={() => { setTabs(prev => prev.map(t => t.id === activeTabId ? {...t, panes: t.panes.map(p => p.id === t.activePaneId ? {...p, blocks: []} : p)} : t)); }} title="Clear Terminal"><Trash2 size={14} /></button>
      ...
      <nav className="tab-bar">
        ...
        <button className="add-tab-btn" onClick={() => createTab(`Terminal ${tabs.length + 1}`)} title="New Tab"><Plus size={14} /></button>
      </nav>

      <main className="main-layout">
        <PanelGroup orientation="horizontal">
          <Panel defaultSize={showWebview ? 60 : 100}>
            <div className="block-stream-container">
              {activeTab && activeTab.panes.length > 0 ? (
                <PanelGroup orientation={activeTab.orientation}>
                  {activeTab.panes.flatMap((pane, i) => {
                          ...
                          <Virtuoso
                            ...
                            itemContent={(_index, block) => (
                              <div key={block.id} className="block">
                                ...
                                {block.status === "completed" && block.type === "shell" && block.output.trim().startsWith("[") && block.output.trim().endsWith("]") && (() => { try { const parsed = JSON.parse(block.output); if (Array.isArray(parsed) && parsed.length > 0 && typeof parsed[0] === "object") { const keys = Object.keys(parsed[0]).slice(0, 6); return (<div className="smart-visualizer"><table className="sv-table"><thead><tr>{keys.map(k => <th key={k}>{k}</th>)}</tr></thead><tbody>{parsed.slice(0, 15).map((row, rIdx) => (<tr key={rIdx}>{keys.map(k => <td key={k}>{String(row[k]).slice(0, 50)}</td>)}</tr>))}</tbody></table>{parsed.length > 15 && <div className="sv-more">... and {parsed.length - 15} more rows</div>}</div>); } } catch {} return null; })()}
                                ...
                                {block.type === "shell" && block.status === "completed" && block.output.toLowerCase().includes("error") && !block.analysis && (
                                  <div className="error-actions-group"><button className="error-analyze-btn" onClick={() => handleAnalyzeError(block)}><Zap size={12} /> AI Analyze</button><button className="error-autofix-btn" onClick={() => handleAutoFix(block)}><Play size={12} /> Auto-Fix</button></div>
                                )}
                              </div>
                            )}
                          />
                    ...
                    if (i < activeTab.panes.length - 1) {
                      elements.push(<PanelResizeHandle key={`handle-${pane.id}`} className="pane-resize-handle" />);
                    }
                    return elements;
                  })}
                </PanelGroup>
          ...
          {showWebview && <PanelResizeHandle className="pane-resize-handle" />}
          {showWebview && (
            <Panel defaultSize={40}>
              <div className="webview-container">
                <div className="webview-toolbar">
                  <div className="webview-address-bar"><Globe size={12} /><input type="text" value={webviewInput} onChange={e => setWebviewUrlInput(e.target.value)} onKeyDown={e => e.key === "Enter" && setWebviewUrl(webviewInput)} /></div>
                  <button className="webview-refresh" onClick={() => { const old = webviewUrl; setWebviewUrl(""); setTimeout(() => setWebviewUrl(old), 10); }}><RefreshCcw size={14} /></button>
                  <button className="webview-close" onClick={() => setShowWebview(false)}><X size={14} /></button>
                </div>
                <iframe src={webviewUrl} className="webview-frame" title="Visual Context" />
              </div>
            </Panel>
          )}
        </PanelGroup>
              ...
              {settingsTab === "general" ? (
                <div className="settings-view">
                  <div className="setting-item">
                    ...
                    <button className={`index-btn ${isIndexing ? "indexing" : ""}`} onClick={handleIndexProject} disabled={isIndexing || !ollamaOnline}>
                      {isIndexing ? <Loader2 size={14} className="animate-pulse" /> : <BrainCircuit size={14} />}
                      {isIndexing ? "Indexing Codebase..." : "Index Codebase (Full RAG)"}
                    </button>
                    {indexProgress && (
                      <div className="index-progress-mini">
                        <div className="progress-bar-bg"><div className="progress-bar-fill" style={{ width: `${(indexProgress.current / indexProgress.total) * 100}%` }} /></div>
                        <div className="index-progress-text">{indexProgress.current} / {indexProgress.total} chunks</div>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="setting-item"><label>Accent Color</label><div className="color-presets">{["#00d4aa", "#a78bfa", "#58a6ff", "#f85149", "#d29922"].map(color => (<div key={color} className={`color-swatch ${config.accent_color === color ? "active" : ""}`} style={{ background: color }} onClick={() => updateConfig({ accent_color: color })} />))}</div></div>
                </div>
              ) : (
                <div className="settings-view models-manager">
                  {pullProgress && (<div className="pull-progress-card"><div className="pull-status">{pullProgress.status}...</div>{pullProgress.total && pullProgress.completed && (<div className="progress-bar-bg"><div className="progress-bar-fill" style={{ width: `${(pullProgress.completed / pullProgress.total) * 100}%` }} /></div>)}</div>)}
                  <div className="model-section"><h4>Installed Models</h4><div className="model-list">{models.map(m => (<div key={m} className="model-item"><HardDrive size={14} className="model-icon" /><span className="model-name">{m}</span><button className="model-delete-btn" onClick={() => handleDeleteModel(m)}><Trash2 size={14} /></button></div>))}</div></div>
                  <div className="model-section"><h4>Available to Download</h4><div className="model-list">{POPULAR_MODELS.map(pm => (<div key={pm.name} className="model-item available"><div className="model-info"><span className="model-name">{pm.name} <small>({pm.size})</small></span><span className="model-desc">{pm.desc}</span></div>{models.includes(pm.name) || (models.some(m => m.startsWith(pm.name))) ? (<span className="model-badge">Installed</span>) : (<button className="model-download-btn" onClick={() => handlePullModel(pm.name)} disabled={!!pullProgress}><Download size={14} /></button>)}</div>))}</div></div>
                </div>
              )}
            </div>
          </div>
