...
import { Copy, RotateCcw, Zap, Minus, Square, X, Trash2, Settings, Plus, Search, Columns, Rows, CheckCircle2, Play, FilePlus, Download, HardDrive, Loader2, Command, History, File, Layers, BrainCircuit } from "lucide-react";
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        setConfig(savedConfig); applyTheme(savedConfig);
        ...
        if (savedTabs && savedTabs.length > 0) {
          ...
          for (const tab of savedTabs) { for (const pane of tab.panes) { await invoke("spawn_pty", { tabId: pane.id, cwd: pane.blocks.slice(-1)[0]?.cwd }); } }
  ...
  const handleCommand = useCallback(async (cmd: string, type: "shell" | "ai") => {
    ...
    try {
      if (type === "ai") {
        ...
        const fullContext = `Project Architecture Summary:\n${context.project_summary}\n\nRelevant Code Snippets:\n${codebaseContext}\n\nCurrent CWD: ${context.cwd}`;
