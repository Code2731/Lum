...
describe("InspectorSummaryOverviewCard", () => {
  it("workspace와 model 기본 정보를 렌더링한다", () => {
    ...
    expect(screen.getByText("바로 이어서")).toBeInTheDocument();
    expect(screen.getByText("세션 복귀")).toBeInTheDocument();
    expect(screen.getByText("RAG 분석")).toBeInTheDocument();
    ...
    expect(screen.getByText("정리된 작업공간에서 바로 분석과 수정을 이어갑니다.")).toBeInTheDocument();
  ...
  it("실패 블록이 있으면 복구 우선 상태와 실패 확인 액션을 노출한다", () => {
    ...
    expect(screen.getByText("복구 시작")).toBeInTheDocument();
  ...
  it("브랜치와 변경 개수가 있으면 배지를 함께 보여준다", () => {
    ...
    expect(screen.getByText("변경 검토")).toBeInTheDocument();
  ...
  it("브랜치가 없으면 브랜치 배지를 숨긴다", () => {
    ...
    expect(screen.getByText("브랜치 정보 없이 열린 탭이라 현재 문맥 확인이 먼저 필요합니다.")).toBeInTheDocument();
  });
  ...
  it("추천 다음 액션 버튼은 각 콜백을 호출한다", () => {
    ...
    screen.getByRole("button", { name: "작업공간" }).click();
