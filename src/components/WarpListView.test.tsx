...
describe("WarpListView pointer outside helper", () => {
  it("target이 null이면 안전하게 false를 반환한다", () => {
    expect(isPointerOutsideTargets(null, [])).toBe(false);
  });
  ...
  it("모든 대상 바깥일 때만 true를 반환한다", () => {
    ...
    expect(isPointerOutsideTargets(child, [container])).toBe(false);
    expect(isPointerOutsideTargets(outside, [container])).toBe(true);
...
describe("WarpListView delta actions", () => {
  ...
  it("Δ Timeline 검색창 단축키는 Ctrl+Alt+L 조합에서 동작하지 않는다", () => {
    render(
      <WarpListView
        blocks={[
          ...blocks,
  ...
  it("Δ Timeline 필터 리셋 단축키(Ctrl+R)", () => {
    render(
      <WarpListView
        blocks={[
          ...blocks,
  ...
  it("Δ Timeline 필터 리셋 단축키는 Ctrl+Alt+R 조합에서 동작하지 않는다", () => {
    render(
      <WarpListView
        blocks={[
          ...blocks,
  ...
  it("Δ Timeline 리스크 필터 단축키(Ctrl+1/2/3/0)", () => {
    render(
      <WarpListView
        blocks={[
          ...blocks,
  ...
  it("Δ Timeline 단축키 도움말 토글은 Slash 코드 경로(Ctrl+code=Slash)도 지원한다", () => {
    render(
      <WarpListView
        blocks={blocks}
        compareResultByBlock={{
    ...
    fireEvent.click(screen.getByRole("button", { name: "Δ Timeline (1)" }));
    fireEvent.keyDown(window, { key: "Unidentified", code: "Slash", ctrlKey: true });
    expect(screen.getByText("타임라인 열기/닫기")).toBeInTheDocument();
    fireEvent.keyDown(window, { key: "Unidentified", code: "Slash", ctrlKey: true });
    expect(screen.queryByText("타임라인 열기/닫기")).not.toBeInTheDocument();
  });
  ...
  it("Δ Timeline 선택 항목 복사 단축키(Ctrl+C)", () => {
    const writeText = vi.fn().mockResolvedValue(undefined);
    Object.defineProperty(navigator, "clipboard", {
      value: { writeText },
      configurable: true,
  ...
  it("Δ Timeline 선택 항목 복사 단축키는 Ctrl+Alt+C 조합에서 동작하지 않는다", () => {
    const writeText = vi.fn().mockResolvedValue(undefined);
    Object.defineProperty(navigator, "clipboard", {
      value: { writeText },
      configurable: true,
  ...
  it("Δ Timeline 전체 복사 단축키(Ctrl+Shift+C)", () => {
    const writeText = vi.fn().mockResolvedValue(undefined);
    Object.defineProperty(navigator, "clipboard", {
      value: { writeText },
      configurable: true,
  ...
  it("Δ Timeline 선택 반전 단축키는 Ctrl+Alt+I 조합에서 동작하지 않는다", () => {
    render(
      <WarpListView
        blocks={[
          ...blocks,
  ...
  it("Δ Timeline 선택 전체 단축키는 Ctrl+Alt+A 조합에서 동작하지 않는다", () => {
    render(
      <WarpListView
        blocks={[
          ...blocks,
  ...
  it("Δ Timeline 선택 항목 핀/핀해제 단축키(Alt+J/Alt+Shift+J)", () => {
    render(
      <WarpListView
        blocks={blocks}
        compareResultByBlock={{
  ...
  it("Δ Timeline 핀만 보기 단축키(Ctrl+M)", () => {
    render(
      <WarpListView
        blocks={[
          ...blocks,
  ...
  it("Δ Timeline 핀만 보기 단축키는 Ctrl+Alt+M 조합에서 동작하지 않는다", () => {
    render(
      <WarpListView
        blocks={[
          ...blocks,
  ...
  it("Δ Timeline 필터 핀/핀해제 버튼", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Δ Timeline (2)" }));
    fireEvent.click(screen.getByRole("button", { name: "필터 핀" }));
    expect(screen.getAllByText("PIN")).toHaveLength(2);

    fireEvent.click(screen.getByRole("button", { name: "필터 핀해제" }));
    expect(screen.queryAllByText("PIN")).toHaveLength(0);
  });

  it("Δ Timeline 필터 핀 단축키(Alt+Shift+P)", () => {
    render(
      <WarpListView
        blocks={[
          ...blocks,
  ...
  it("Δ Timeline 필터 핀 단축키(Ctrl+Shift+P)", () => {
    render(
      <WarpListView
        blocks={[
          ...blocks,
  ...
  it("Δ Timeline 필터 핀 단축키는 Ctrl+Alt+Shift+P 조합에서 동작하지 않는다", () => {
    render(
      <WarpListView
        blocks={[
          ...blocks,
    ...
    fireEvent.click(screen.getByRole("button", { name: "Δ Timeline (2)" }));
    fireEvent.keyDown(window, { key: "P", ctrlKey: true, altKey: true, shiftKey: true });
    expect(screen.queryAllByText("PIN")).toHaveLength(0);
  });

  it("Δ Timeline 필터 핀해제 단축키(Alt+Shift+O)", () => {
    render(
      <WarpListView
        blocks={[
          ...blocks,
    ...
    fireEvent.keyDown(window, { key: "O", altKey: true, shiftKey: true });
    expect(screen.queryAllByText("PIN")).toHaveLength(0);
  });

  it("Δ Timeline 필터 핀해제 단축키(Ctrl+Shift+O)", () => {
    render(
      <WarpListView
        blocks={[
          ...blocks,
  ...
  it("Δ Timeline 핀 전체 해제 단축키(Ctrl+Shift+U)", () => {
    render(
      <WarpListView
        blocks={[
          ...blocks,
  ...
  it("Δ Timeline 핀 전체 해제 단축키는 Ctrl+Alt+Shift+U 조합에서 동작하지 않는다", () => {
    render(
      <WarpListView
        blocks={[
          ...blocks,
    ...
    fireEvent.keyDown(window, { key: "U", ctrlKey: true, altKey: true, shiftKey: true });
    expect(screen.queryByText("$ npm test")).not.toBeInTheDocument();
