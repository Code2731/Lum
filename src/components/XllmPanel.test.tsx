...
describe("XllmPanel", () => {
  ...
  it("상단 설정 저장 실패 시 객체 message가 숫자면 문자열로 노출한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
      ...
      if (cmd === "save_xllm_settings") return Promise.reject({ message: 503 });
      if (cmd === "save_recall_vector_backend") return Promise.resolve(args ?? {});
      if (cmd === "list_embed_candidates") return Promise.resolve([]);
      if (cmd === "list_lora_candidates") return Promise.resolve([]);
      if (cmd === "embed_loaded_info") return Promise.resolve(null);
  ...
  it("상단 설정 저장 실패 시 객체 message가 Error여도 내부 메시지를 노출한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
      ...
      if (cmd === "save_xllm_settings") return Promise.reject({ message: new Error("중첩 오류") });
      if (cmd === "save_recall_vector_backend") return Promise.resolve(args ?? {});
      if (cmd === "list_embed_candidates") return Promise.resolve([]);
      if (cmd === "list_lora_candidates") return Promise.resolve([]);
      if (cmd === "embed_loaded_info") return Promise.resolve(null);
    ...
    expect(await screen.findByText("저장 실패: 중첩 오류")).toBeInTheDocument();
  });

  it("상단 설정 저장 실패 시 객체 error 필드 문자열도 메시지로 노출한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
      ...
      if (cmd === "save_xllm_settings") return Promise.reject({ error: "백엔드 에러 필드" });
      if (cmd === "save_recall_vector_backend") return Promise.resolve(args ?? {});
      if (cmd === "list_embed_candidates") return Promise.resolve([]);
      if (cmd === "list_lora_candidates") return Promise.resolve([]);
      if (cmd === "embed_loaded_info") return Promise.resolve(null);
    ...
    expect(await screen.findByText("저장 실패: 백엔드 에러 필드")).toBeInTheDocument();
  });

  it("상단 설정 저장 실패 시 객체 error 필드가 Error여도 내부 메시지를 노출한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
      ...
      if (cmd === "save_xllm_settings") return Promise.reject({ error: new Error("중첩 error 필드") });
      if (cmd === "save_recall_vector_backend") return Promise.resolve(args ?? {});
      if (cmd === "list_embed_candidates") return Promise.resolve([]);
      if (cmd === "list_lora_candidates") return Promise.resolve([]);
      if (cmd === "embed_loaded_info") return Promise.resolve(null);
    ...
    expect(await screen.findByText("저장 실패: 중첩 error 필드")).toBeInTheDocument();
  });

  it("상단 설정 저장 실패 시 message가 빈 문자열이면 error 필드로 폴백한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
      ...
      if (cmd === "save_xllm_settings") {
        return Promise.reject({ message: "   ", error: "error 필드 폴백" });
      }
      if (cmd === "save_recall_vector_backend") return Promise.resolve(args ?? {});
      if (cmd === "list_embed_candidates") return Promise.resolve([]);
      if (cmd === "list_lora_candidates") return Promise.resolve([]);
    ...
    expect(await screen.findByText("저장 실패: error 필드 폴백")).toBeInTheDocument();
  });

  it("상단 설정 저장 실패 시 message가 비어 있으면 errorMessage 필드로 폴백한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
      ...
      if (cmd === "save_xllm_settings") {
        return Promise.reject({ message: " ", errorMessage: "errorMessage 폴백" });
      }
      if (cmd === "save_recall_vector_backend") return Promise.resolve(args ?? {});
      if (cmd === "list_embed_candidates") return Promise.resolve([]);
      if (cmd === "list_lora_candidates") return Promise.resolve([]);
    ...
    expect(await screen.findByText("저장 실패: errorMessage 폴백")).toBeInTheDocument();
  });

  it("상단 설정 저장 실패 시 message/errorMessage가 비어 있으면 error_description으로 폴백한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
      ...
      if (cmd === "save_xllm_settings") {
        return Promise.reject({
          message: "",
          errorMessage: " ",
          error_description: "error_description 폴백",
        });
      }
      if (cmd === "save_recall_vector_backend") return Promise.resolve(args ?? {});
      if (cmd === "list_embed_candidates") return Promise.resolve([]);
    ...
    expect(await screen.findByText("저장 실패: error_description 폴백")).toBeInTheDocument();
  });

  it("상단 설정 저장 실패 시 snake_case가 비어 있으면 errorDescription으로 폴백한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
      ...
      if (cmd === "save_xllm_settings") {
        return Promise.reject({
          ...
          errorMessage: " ",
          error_description: "\n",
          errorDescription: "errorDescription 폴백",
        });
      }
      if (cmd === "save_recall_vector_backend") return Promise.resolve(args ?? {});
      if (cmd === "list_embed_candidates") return Promise.resolve([]);
    ...
    expect(await screen.findByText("저장 실패: errorDescription 폴백")).toBeInTheDocument();
  });

  it("상단 설정 저장 실패 시 details 배열에서 첫 유효 메시지를 추출한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
      ...
      if (cmd === "save_xllm_settings") {
        return Promise.reject({
          ...
          error: " ",
          detail: " ",
          details: [{ msg: " " }, { reason: "details 배열 reason 메시지" }],
        });
      }
      if (cmd === "save_recall_vector_backend") return Promise.resolve(args ?? {});
      if (cmd === "list_embed_candidates") return Promise.resolve([]);
    ...
    expect(await screen.findByText("저장 실패: details 배열 reason 메시지")).toBeInTheDocument();
  });

  it("상단 설정 저장 실패 시 statusText/status가 비어 있으면 code를 폴백한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
    ...
    expect(await screen.findByText("저장 실패: ECONNREFUSED")).toBeInTheDocument();
  });

  it("상단 설정 저장 실패 시 message가 비어 있으면 messages 배열의 첫 유효 값을 사용한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
    ...
    expect(await screen.findByText("저장 실패: messages 배열 폴백")).toBeInTheDocument();
  });

  it("상단 설정 저장 실패 시 details까지 비어 있으면 description으로 폴백한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
    ...
    expect(await screen.findByText("저장 실패: description 폴백")).toBeInTheDocument();
  });

  it("상단 설정 저장 실패 시 description까지 비어 있으면 title로 폴백한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
    ...
    expect(await screen.findByText("저장 실패: title 폴백")).toBeInTheDocument();
  });

  it("상단 설정 저장 실패 시 message와 error/cause가 함께 있으면 message를 우선 사용한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
    ...
    expect(await screen.findByText("저장 실패: message 우선")).toBeInTheDocument();
  });

  it("상단 설정 저장 실패 시 message와 error가 비어 있으면 cause로 폴백한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
      ...
      if (cmd === "save_xllm_settings") {
        return Promise.reject({ message: "", error: "   ", cause: "cause 폴백" });
      }
      if (cmd === "save_recall_vector_backend") return Promise.resolve(args ?? {});
      if (cmd === "list_embed_candidates") return Promise.resolve([]);
      if (cmd === "list_lora_candidates") return Promise.resolve([]);
    ...
    expect(await screen.findByText("저장 실패: cause 폴백")).toBeInTheDocument();
  });

  it("상단 설정 저장 실패 시 message/error/cause가 비어 있으면 detail로 폴백한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
      ...
      if (cmd === "save_xllm_settings") {
        return Promise.reject({ message: "", error: "  ", cause: "   ", detail: "detail 폴백" });
      }
      if (cmd === "save_recall_vector_backend") return Promise.resolve(args ?? {});
      if (cmd === "list_embed_candidates") return Promise.resolve([]);
      if (cmd === "list_lora_candidates") return Promise.resolve([]);
    ...
    expect(await screen.findByText("저장 실패: detail 폴백")).toBeInTheDocument();
  });

  it("상단 설정 저장 실패 시 detail까지 비어 있으면 reason으로 폴백한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
    ...
    expect(await screen.findByText("저장 실패: reason 폴백")).toBeInTheDocument();
  });

  it("상단 설정 저장 실패 시 reason까지 비어 있으면 msg로 폴백한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
    ...
    expect(await screen.findByText("저장 실패: msg 폴백")).toBeInTheDocument();
  });

  it("상단 설정 저장 실패 시 detail 배열의 msg를 파싱해 노출한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
    ...
    expect(await screen.findByText("저장 실패: 배열 detail 메시지")).toBeInTheDocument();
  });

  it("상단 설정 저장 실패 시 detail 배열에서 첫 유효 메시지를 선택한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
      ...
      if (cmd === "save_xllm_settings") {
        return Promise.reject({
          ...
          error: " ",
          cause: " ",
          detail: [{ msg: "   " }, { reason: "두번째 reason 메시지" }, { msg: "세번째" }],
        });
      }
      if (cmd === "save_recall_vector_backend") return Promise.resolve(args ?? {});
      if (cmd === "list_embed_candidates") return Promise.resolve([]);
  ...
  it("상단 설정 저장 실패 시 message가 비어 있고 error/cause가 있으면 error를 우선 사용한다", async () => {
    ...
    expect(await screen.findByText("저장 실패: error 우선")).toBeInTheDocument();
  });

  it("상단 설정 저장 실패 시 message/error가 비어 있으면 errors 배열로 폴백한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
    ...
    expect(await screen.findByText("저장 실패: errors 배열 폴백")).toBeInTheDocument();
  });

  it("상단 설정 저장 실패 시 errors 배열 객체 항목에서도 메시지를 추출한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
    ...
    expect(await screen.findByText("저장 실패: errors 객체 항목 메시지")).toBeInTheDocument();
  });

  it("상단 설정 저장 실패 시 response.data 중첩 객체에서 메시지를 추출한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
    ...
    expect(await screen.findByText("저장 실패: response.data 중첩 메시지")).toBeInTheDocument();
  });

  it("상단 설정 저장 실패 시 body 배열에서도 첫 유효 메시지를 추출한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
      ...
      if (cmd === "save_xllm_settings") {
        return Promise.reject({
          ...
          error: " ",
          errors: [],
          body: [{ msg: " " }, { reason: "body 배열 reason 메시지" }],
        });
      }
      if (cmd === "save_recall_vector_backend") return Promise.resolve(args ?? {});
      if (cmd === "list_embed_candidates") return Promise.resolve([]);
    ...
    expect(await screen.findByText("저장 실패: body 배열 reason 메시지")).toBeInTheDocument();
  });

  it("상단 설정 저장 실패 시 statusText 필드 문자열을 폴백 메시지로 사용한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
    ...
    expect(await screen.findByText("저장 실패: Service Unavailable")).toBeInTheDocument();
  });

  it("상단 설정 저장 실패 시 statusText가 비어 있으면 status 숫자를 폴백한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
    ...
    expect(await screen.findByText("저장 실패: 429")).toBeInTheDocument();
  });

  it("상단 설정 저장 실패 시 객체 error 필드가 숫자면 문자열로 노출한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
      ...
      if (cmd === "save_xllm_settings") return Promise.reject({ error: 503 });
      if (cmd === "save_recall_vector_backend") return Promise.resolve(args ?? {});
      if (cmd === "list_embed_candidates") return Promise.resolve([]);
      if (cmd === "list_lora_candidates") return Promise.resolve([]);
      if (cmd === "embed_loaded_info") return Promise.resolve(null);
  ...
  it("상단 설정 저장 실패 시 bigint 오류를 문자열로 노출한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
      ...
      if (cmd === "save_xllm_settings") return Promise.reject(9007199254740993n);
      if (cmd === "save_recall_vector_backend") return Promise.resolve(args ?? {});
      if (cmd === "list_embed_candidates") return Promise.resolve([]);
      if (cmd === "list_lora_candidates") return Promise.resolve([]);
      if (cmd === "embed_loaded_info") return Promise.resolve(null);
    ...
    expect(await screen.findByText("저장 실패: 9007199254740993")).toBeInTheDocument();
  });

  it("상단 설정 저장 실패 시 symbol 오류를 문자열로 노출한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
      ...
      if (cmd === "save_xllm_settings") return Promise.reject(Symbol("network"));
      if (cmd === "save_recall_vector_backend") return Promise.resolve(args ?? {});
      if (cmd === "list_embed_candidates") return Promise.resolve([]);
      if (cmd === "list_lora_candidates") return Promise.resolve([]);
      if (cmd === "embed_loaded_info") return Promise.resolve(null);
    ...
    expect(await screen.findByText("저장 실패: Symbol(network)")).toBeInTheDocument();
  });

  it("상단 설정 저장 실패 시 공백 문자열 오류는 기본 문구로 노출한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
  ...
  it("상단 설정 저장 실패 시 JSON 문자열 오류에서 내부 message를 추출한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
      ...
      if (cmd === "save_xllm_settings") {
        return Promise.reject('{ "message": "JSON 문자열 내부 메시지" }');
      }
      if (cmd === "save_recall_vector_backend") return Promise.resolve(args ?? {});
      if (cmd === "list_embed_candidates") return Promise.resolve([]);
      if (cmd === "list_lora_candidates") return Promise.resolve([]);
    ...
    expect(await screen.findByText("저장 실패: JSON 문자열 내부 메시지")).toBeInTheDocument();
  });

  it("상단 설정 저장 실패 시 잘못된 JSON 문자열 오류는 원문 문자열을 유지한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
      ...
      if (cmd === "save_xllm_settings") return Promise.reject("{ broken json");
      if (cmd === "save_recall_vector_backend") return Promise.resolve(args ?? {});
      if (cmd === "list_embed_candidates") return Promise.resolve([]);
      if (cmd === "list_lora_candidates") return Promise.resolve([]);
      if (cmd === "embed_loaded_info") return Promise.resolve(null);
    ...
    expect(await screen.findByText("저장 실패: { broken json")).toBeInTheDocument();
  });

  it("상단 설정 저장 실패 시 알려진 키가 없어도 커스텀 필드의 메시지를 추출한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
    ...
    expect(await screen.findByText("저장 실패: 커스텀 필드 메시지")).toBeInTheDocument();
  });

  it("상단 설정 저장 실패 시 JSON 문자열의 커스텀 배열 필드도 추출한다", async () => {
    invokeMock.mockReset();
    invokeMock.mockImplementation((cmd: string, args?: unknown) => {
      if (cmd === "load_app_config") return Promise.resolve({});
      if (cmd === "recall_backend_info") {
      ...
      if (cmd === "save_xllm_settings") {
        return Promise.reject('{ "payload": [{ "note": " " }, { "note": "JSON 커스텀 메시지" }] }');
      }
      if (cmd === "save_recall_vector_backend") return Promise.resolve(args ?? {});
      if (cmd === "list_embed_candidates") return Promise.resolve([]);
      if (cmd === "list_lora_candidates") return Promise.resolve([]);
