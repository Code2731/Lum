...
describe("useInspectorPanelCommands", () => {
  ...
  ])("명시한 블록 ID가 $label이면 정규화 후 최근 실패 블록으로 분석 프롬프트를 생성한다", ({ blockId }) => {
    const { result, spies } = setupInspectorCommands({
      cmdBlocks: [
        buildBlock("ok-1", "pwd", "ok", 0),
        buildBlock("fail-1", "cat missing", "err", 1),
  ...
  ])("선택 블록이 $label이면 정규화 후 최근 실패 블록으로 분석 프롬프트를 생성한다", ({ selectedBlockId }) => {
    const { result, spies } = setupInspectorCommands({
      cmdBlocks: [
        buildBlock("ok-1", "pwd", "ok", 0),
        buildBlock("fail-1", "cat missing", "err", 1),
  ...
  it.each([
    ...
    { label: "미존재 탭", blockId: "\tbad-3\t" },
    { label: "미존재 BOM", blockId: "\uFEFFbad-3" },
  ])("블록 ID에 $label이 섞여도 미존재 ID는 최근 실패 블록을 대상으로 복사한다", async ({ blockId }) => {
    const { result } = setupInspectorCommands({
      cmdBlocks: [
        buildBlock("ok-1", "pwd", "ok", 0),
        buildBlock("fail-1", "cat missing", "err", 1),
  ...
  it("handleInspectorSuggestedCommandRowKeyDown에서 메뉴 행이 다르면 아무 동작도 하지 않는다", () => {
    const { result, spies } = setupInspectorCommands({
      inspectorAnalyzeCache: {
        blockId: "b",
        command: "bad",
    ...
    act(() => {
      result.current.handleInspectorSuggestedCommandRowKeyDown(e, 1);
    });
  ...
  it("handleInspectorSuggestedCommandRowKeyDown에서 메뉴 행이 다르면 copy/load 키도 처리하지 않는다", () => {
    const { result, spies } = setupInspectorCommands({
      inspectorAnalyzeCache: {
        blockId: "b",
        command: "bad",
    ...
    act(() => {
      result.current.handleInspectorSuggestedCommandRowKeyDown(eCopy, 1);
      result.current.handleInspectorSuggestedCommandRowKeyDown(eLoad, 1);
    });
  ...
  it("handleInspectorSuggestedCommandRowKeyDown에서 메뉴 인덱스가 null이면 아무 동작도 하지 않는다", () => {
    const { result, spies } = setupInspectorCommands({
      inspectorAnalyzeCache: {
        blockId: "b",
        command: "bad",
  ...
  it("handleInspectorSuggestedCommandRowKeyDown에서 수정자 키가 있으면 매핑되어도 동작하지 않는다", () => {
    const { result, spies } = setupInspectorCommands({
      inspectorAnalyzeCache: {
        blockId: "b",
        command: "bad",
