...
async function emitAnalysisResult(
  tokenHandlers: TokenHandler[],
  ...
  await act(async () => {
    tokenHandlers[0]?.({ payload });
    finishMockStream(resolveStream);
    await Promise.resolve();
  });
}

function holdMockStream(
  mockedInvoke: ReturnType<typeof vi.mocked<typeof invoke>>,
): () => void {
...
describe("App (LUM 터미널)", () => {
  ...
  it("실패 블록 포커스가 공백-only ID를 가질 때도 AI ANALYZE가 STREAMING으로 진입해야 한다", async () => {
    const releaseStream = holdMockStream(mockedInvoke);
    setMockCommandBlocks([
      {
        id: "cmd-1",
        command: "npm test",
  ...
  it("입력 포커스 중에는 Cmd+Shift+L이 스크립트 패널 토글로 처리되지 않는다", async () => {
  ...
  it("입력 포커스 중에는 Ctrl+,가 테마 패널 토글로 처리되지 않는다", () => {
    const input = document.createElement("input");
    document.body.appendChild(input);
  ...
  it("글로벌 단축키는 입력 필드 포커스 시 동작하지 않는다(Ctrl/Cmd)", async () => {
    ...
    const shortcuts = [
      ...
      { name: "탭 닫기", key: "w", ctrlKey: true },
      { name: "탭 닫기", key: "w", metaKey: true },
      ...
      { name: "실패 블록 포커스", key: "f", ctrlKey: true, shiftKey: true },
      { name: "실패 블록 포커스", key: "f", metaKey: true, shiftKey: true },
      { name: "워크스페이스 패널", key: "o", ctrlKey: true, shiftKey: true },
      { name: "워크스페이스 패널", key: "o", metaKey: true, shiftKey: true },
      { name: "워크스페이스 패널", key: "s", ctrlKey: true, shiftKey: true },
      { name: "워크스페이스 패널", key: "s", metaKey: true, shiftKey: true },
      { name: "블록 이동(+Shift+ArrowUp)", key: "ArrowUp", ctrlKey: true, shiftKey: true },
      { name: "블록 이동(+Shift+ArrowDown)", key: "ArrowDown", ctrlKey: true, shiftKey: true },
  ...
  it("readonly textarea 포커스에서는 글로벌 단축키가 동작한다", async () => {
    render(<App />);
  ...
  it("aria-disabled role=textbox에서는 글로벌 단축키가 동작한다", async () => {
    render(<App />);
  ...
  it("aria-disabled contenteditable plaintext-only에서는 글로벌 단축키가 동작한다", async () => {
    render(<App />);
  ...
  it("role=textbox + aria-disabled + contenteditable='plaintext-only' 에서 글로벌 단축키가 동작한다", async () => {
    render(<App />);
  ...
  it("role=searchbox + aria-disabled에서 글로벌 단축키가 동작한다", async () => {
    render(<App />);
  ...
  it("분석 결과 전체 복사 버튼은 분석 요약을 클립보드에 반영한다", async () => {
    ...
    try {
      ...
      expect(writeText.mock.calls[0]?.[0]).toContain("npm run test");
    } finally {
      mockedListen.mockImplementation(mockedBaseListen as any);
      mockedInvoke.mockImplementation(mockedBaseInvoke as any);
      Object.defineProperty(navigator, "clipboard", { configurable: true, value: baseClipboard });
  ...
  it("실패 블록이 없을 때 요약 패널은 실패 전용 액션을 노출하지 않는다", async () => {
    setMockCommandBlocks([{
      id: "ok-1",
      command: "npm test",
      output: "All good",
