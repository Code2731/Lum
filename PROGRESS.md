...
- 복구 UX 흐름 1차 고도화
  - 헤더 → 알림 센터 → 인스펙터로 이어지는 복구 진입 동선을 더 직접적으로 정리
  - healing 알림/실패 블록/분석 결과/빠른 작업 카드가 같은 복구 언어(`복구 시작`, `첫 제안 실행`, `인스펙터 연계`)를 쓰도록 정합성 보강
- 인스펙터 복구 흐름 요약 강화
  - `src/hooks/useInspectorPanelPropsBundle.ts`
    - `getInspectorPanelPropsBundleMeta(...)` 추가
  - `src/App.tsx`
    - 인스펙터 오픈 시 상단 상태 요약 바 추가
  - `src/components/InspectorPanelSummary.tsx`
    - active flow 요약 + 1차 CTA(`실패 분석 시작`, `실패 블록 다시 보기`, `최근 기록 열기`) 추가
- 실패/분석 카드의 복구 시작점 강화
  - `src/components/InspectorFailedBlockCard.tsx`
    - `먼저 실패 확인 → 다음 분석 열기 → 마지막 로그/프롬프트` 흐름 배지와 `바로 복구 시작` 안내 추가
  - `src/components/InspectorAnalyzeCard.tsx`
    - 추천 커맨드 우선순위 배지(`먼저 실행`, `대안`, `추가 점검`) 추가
    - 첫 제안 빠른 액션 바(`실행/복사/입력`)와 첫 제안/다음 대안 미리보기 추가
- 빠른 작업/요약 카드의 복구 언어 정렬
  - `src/components/InspectorQuickActionsCard.tsx`
    - 고급 영역에 `바로 복구 시작 → 실패 분석 연결 → 첫 제안 실행` 흐름 바 추가
  - `src/components/InspectorSummaryOverviewCard.tsx`
    - 실패 우선 상태의 주요 CTA를 `실패 확인`에서 `복구 시작`으로 정렬
- 알림 센터 복구 진입 고도화
  - `src/components/NotificationCenter.tsx`
    - healing 알림 상단 복구 요약 바 추가
    - healing 카드에 `바로 복구 보기` 가이드, 우선순위 배지, `인스펙터 열기` 버튼 추가
    - `인스펙터 열기` 클릭 시 읽음 처리와 복구 진입을 함께 수행
    - 헤더 복구 배지로 열렸을 때 healing 알림을 목록 상단에 우선 정렬
- 헤더 복구 배지 고도화
  - `src/components/AppHeader.tsx`
    - healing 알림 기반 복구 배지 추가
    - 배지를 클릭 가능한 진입점으로 변경해 알림 센터 오픈
    - unread healing이 있을 때 강조 스타일 + ping 점 추가
    - 툴팁 문구를 실행형 카피로 정리

### 현재 상태 요약

- 복구 흐름이 여러 패널에 흩어져 있던 상태에서, 상단 헤더와 알림 센터를 통해 인스펙터 복구 흐름으로 빠르게 들어가는 경로가 생김
- 실패 블록 확인 이후 분석과 첫 제안 실행으로 이어지는 복구 플로우가 카드/배지/CTA 수준에서 더 일관된 언어를 사용함
- 테스트는 요청이 없어 실행하지 않았고, 변경된 메타/카피/클릭 흐름에 대한 단위 테스트 기대값만 함께 보강

### 다음 추천 라운드

- 알림 센터에서 복구 진입 후 인스펙터 첫 포커스 위치를 더 정교하게 연결
- 복구 흐름 카피를 `README` 또는 사용자 가이드 성격 문서에도 반영
- 필요 시 복구 UX 관련 테스트를 1~2개 핵심 묶음으로 실제 실행해 회귀를 고정
...
1. [x] 남아 있는 경고를 “실제 불필요 코드”와 “타깃별 조건부 경고”로 분류
   - 2026-07-10 기준, `TODO.md` 품질 섹션의 분류 기준(해결군/조건부/보류)과 일치하도록 정리
2. [x] `TODO.md`를 추가/갱신해 MVP 남은 작업을 기능/품질/배포 기준으로 정리
   - 기능/품질/배포 3개 기준으로 남은 항목 뷰를 분리해 다음 라운드 시작 전 우선순위 정렬
3. [x] 다음 추천 라운드에서는 회귀 테스트 1~2개만 묶어 실행
   - 다음 라운드 첫 실행은 `useAIChat` 회귀 1건 + 입력 포커스 회귀 1건, 총 2개 테스트 묶음으로 제한

### 2026-07-10 추천 라운드 반영

- [x] TODO 정리 항목 재구성
  - `MVP 잔여 작업 정리`에 기능/품질/배포 분류 기준을 재정의해 경로별 미완료 항목을 가시화
  - 다음 라운드 진입 전 회귀 테스트 1~2개 묶음 원칙으로 축소할 수 있도록 남은 작업 우선순위를 정렬

### 2026-07-11 추천 라운드 반영

- [x] 회귀 테스트 묶음 원칙 확정
  - 다음 라운드 시작 전 회귀 실행 범위를 `2개 이하`로 고정하고,
  - `useAIChat`/`입력 포커스`를 대표군으로 지정해 고장 전파 가능성을 우선 점검하도록 기준화

### 2026-07-10 추천 라운드 반영

- [x] 경고 분류 정합성 마감
  - `남은 경고`를 기능적으로 `해결군(제거 또는 실제 반영)`와 `조건부 경고(embedded-ai 플래그·특정 빌드 경로)`로 분리해 기록
  - `TODO.md` 권장 라운드 항목과 동기화해 다음 라운드 목표 진입 전 품질 분류 기준을 고정
