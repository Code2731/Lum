...
- 실제 사용자 기준 E2E 확장
  - 현재 `e2e/smoke.spec.ts` `25 passed`
  - 기본 실행/AI 응답/Inspector 추천 커맨드/영속성까지는 커버됨
  - 아직 남은 핵심 사용자 흐름:
  - 우선순위:
    - 첫 실행 직후 단축키/툴바 사용성이 방해 없이 이어지는지 확인
    - 온보딩 이후 첫 실제 작업(새 탭/AI 바/파일 탐색기)이 자연스럽게 이어지는지 확인
- 설정 영속성 최종 확인
  - 툴바 단순 모드
  - Inspector 표시 여부
...
1. 첫 실행 직후 단축키/툴바 사용성 점검
2. 온보딩 이후 첫 실제 작업 연결성 점검
3. 이번 누적 변경분 커밋/푸시
