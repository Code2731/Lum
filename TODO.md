...
- [x] 기능: 남은 경고 분류 마감 후 1~2개 회귀 테스트 묶음 운영 방안 확정(다음 추천 라운드)
  - 다음 라운드부터 `useAIChat` 회귀 + 입력 포커스 회귀를 1~2개 묶음으로 우선 실행
- [x] 기능: `cmd`/`app` 경계 상태에서 폴백 UX의 사용자 메시지 톤 통일 여부 최종 점검
- [x] 배포: Linux CPU 빌드 산출물 자동 검증( smoke ) 스크립트 보강
...
- [x] 회귀 테스트 묶음 상한 정리
  - 다음 라운드부터 회귀 실행 단위를 `1~2개`로 제한
  - 우선 대상군: `useAIChat` 회귀 + 입력 포커스 회귀를 우선 묶음으로 유지
