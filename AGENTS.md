...
- **외부 MCP 클라이언트 연동**: `lum-mcp-server` 바이너리(stdio MCP, 7개 도구) — Codex Desktop / CrewAI 등 외부 agent가 LUM 도구 호출 가능 (LUM 본체는 Python 의존성 없음)
