...
- **RAG** (`commands/rag.rs`): `index_project` / `search_codebase` / `rag_context_for_file` — 소스코드 청킹 + 임베딩 벡터 검색. `embed_auto`: Ollama → xLLM 폴백 자동 선택. Phase 137-C — 증분 인덱싱: `MemoryEntry.file(Option<String>)` + `SemanticMemory.file_mtimes(HashMap<String,u64>)` — mtime 비교로 변경 파일만 재청킹·재임베딩. `diff_files(disk, cached)` 순수 함수가 unchanged/changed/added/deleted 분류. 삭제 파일 청크는 `entries.retain`으로 즉시 제거. 임베딩 성공 파일만 mtime 캐시 갱신(서버 장애 시 다음 run 자동 재시도). `IndexResult`에 `skipped_files`/`removed_files` 필드 추가. 회귀 가드 5건(mtime 동일 스킵·변경 재인덱싱·신규 추가·삭제 제거·혼합).
- **BM25 + RRF 하이브리드 검색** (Phase 138, `commands/bm25.rs`): tantivy 없는 경량 in-memory BM25. `tokenize_code` — snake_case/camelCase/ALL_CAPS/Rust 경로(`::`) 인식 코드 토크나이저. `Bm25Index::build(docs)` + `search(query, limit)` — BM25 k1=1.2/b=0.75. `rrf_fuse(dense, lexical)` — Reciprocal Rank Fusion k=60. `search_codebase` 수정: BM25 lexical top-20 + dense(cosine) top-20 → RRF 융합 → limit 반환. 식별자 정확매칭(`parse_url`, `JWT_SECRET`)은 BM25 강점, 의미 유사도는 dense 강점. 회귀 가드 10건(tokenize 5종·bm25 검색 3건·rrf 2건).
- **호출 그래프 + multi-hop** (Phase 139, `commands/call_graph.rs`): tree-sitter 기반 함수 호출 그래프. `parse_file_for_calls` — fn_def_query(함수 정의+범위) + call_query(call_expression 피호출자)로 innermost 함수에 귀속. `CallGraph::build(root)` — 프로젝트 전체 walk, 3000 파일 제한. `find_callers(symbol)` / `find_callees(symbol)` / `trace_dependents(symbol, depth)` — BFS 순환 방지. ReAct 도구 3종 추가. 출력에 항상 "동명이인 미구분" disclaimer. 회귀 가드 8건(find_callers/callees·trace_dependents BFS·depth 제한·순환·동명이인·외부함수·없을 때 빈 벡터).
- **Personalized PageRank** (Phase 140, `commands/repo_map.rs`): Aider 방식. `build_graph_and_rank`에 `active_file: Option<&Path>` + `mentioned_symbols: &[String]` 추가. 개인화 가중치: active_file 50×, mentioned_symbols 포함 파일 10×, `is_well_named`(snake_case 3+/CamelCase 2+) 심볼 파일 1.5×, 나머지 1×. 가중치 → 정규화 → personalization 벡터 p, dangling node도 p로 재분배. `get_repo_map` 커맨드에 `active_file: Option<String>`, `mentioned_symbols: Option<Vec<String>>` 파라미터 추가(기존 호출 None/None 하위호환). 회귀 가드 4건(is_well_named·active 최우선·mentioned 승격·None 균등).
...
- **Phase 143-A — GraphRAG flat 초기 진입** (`commands/react_agent.rs`): `query_graph` 도구 추가. 의미 검색 결과 상위 심볼을 `parse_graph_symbol_from_chunk`로 추출해 `find_callers`/`find_callees`/`trace_dependents`를 결합하여 호출·영향 요약. 심볼 미존재 시 검색 스니펫만 반환. 회귀 가드 3건(도구 등록·심볼 파싱·빈 심볼 폴백).
- **Phase 136 — Magentic 2-ledger** (`commands/react_agent.rs`): inner Progress Ledger + outer TaskLedger 통합. `ProgressLedger { action_counts, key_facts, stuck_total }` — L1(첫반복: 힌트+facts주입+계속) / L2(stuck_total≥3: outer재계획 시도→강제ANSWER). outer: `is_complex_goal`(20단어+/120자+/복수태스크 표지) 감지 → `generate_task_plan`(LLM호출) → `parse_task_plan`(번호/대시 목록 파싱 최대7단계) → 계획 컨텍스트 주입. L2 escalation 시 `outer_replan_count<2`이면 재계획+stuck_total리셋 → 두 번 재계획 실패시 강제ANSWER. 회귀 가드 17건(is_complex 5+parse 5+ledger 7).
