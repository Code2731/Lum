...
- **Phase 136-A — Progress Ledger + Stuck 회복** (`commands/react_agent.rs`): Magentic-One 방식 inner loop 개선. `ProgressLedger { action_counts, key_facts, stuck_total }` — `record(key)→count`, `absorb_observation(tool, obs)→첫줄 120자 cap 최대8개`, `recovery_l1(key)→힌트+facts 주입`, `recovery_l2()→강제ANSWER 메시지`. 기존 `recent_actions+강제종료` → L1(첫반복: 힌트주입 후 계속) / L2(stuck_total≥3: 강제ANSWER). `absorb_observation`으로 관찰 핵심을 facts에 누적 → recovery 시 컨텍스트 제공. 회귀 가드 7건. outer TaskLedger(orchestrator.rs)는 Phase 136-B에서.
