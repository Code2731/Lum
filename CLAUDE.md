...
- **영속성**: `.lum_session.json`, `.lum_config.json`(Phase 120: `auto_lora_*` 필드 추가), `.lum_code_index.json`, `.lum_ssh_profiles.json`, `.lum_mcp.json`, `.lum_squads.json`, `.lum_healing_dataset.jsonl`, `.lum_lora_runs.json`(Phase 120: `auto_train_cursor_ms` 필드 추가) + `.lum_lora_runs/<id>/`.
