...
| **Multi-Backend Routing** | AI backends (embedded mistral.rs / Ollama / xLLM HTTP / Gemini) **coexist** — they use different resources. `@local` / `@ollama` / `@xllm` (`@sglang` alias) / `@gemini` prefix forces a specific backend per task — mix freely (e.g. `@gemini` for long-context analysis, `@local` for fast function edits). No prefix → automatic fallback chain. |
...
| **Multi-Backend 라우팅** | AI 백엔드(임베디드 mistral.rs / Ollama / xLLM HTTP / Gemini)는 자원이 다르므로 **공존**. `@local` / `@ollama` / `@xllm`(`@sglang` alias) / `@gemini` prefix로 작업별 backend 강제 — `@gemini 큰 컨텍스트 분석`, `@local 빠른 함수 추가` 같이 혼합 사용. 백엔드 미지정 시 자동 fallback chain. |
...
| **125-1** | Multi-Backend prefix | AI 백엔드 공존 인정 — `@local`/`@ollama`/`@xllm`(`@sglang` alias)/`@gemini` prefix로 작업별 backend 강제. 자원 다른 백엔드들을 unload 강제 없이 혼합 사용. 2~3차에 Rust forwarding + UI chip + 자동 정책 |
