...
| Rust 단위 테스트 | 전체 `cargo test --lib` 420건 통과 | 완료 |
...
| Whisper 실행 환경 | 음성 프론트 6건·Rust 27건 통과. Homebrew `whisper-cli`와 `ggml-base.bin`(SHA-1 검증)을 설치했고, 통제된 한국어 기준 음성을 LUM UI 입력창까지 연결해 확인함. 고유명사 인식은 베타 수준 | 완료(베타 정확도) |
...
- LUM의 `@local` 임베디드 응답, `@xllm` 외부 서버 응답, `@ollama` 응답은 실제 UI에서 입증됐습니다. 음성은 LUM UI에서 마이크 시작·중지, `듣는 중` 상태, 통제된 한국어 전사 결과의 입력창 삽입까지 실제로 입증됐고, `LUM` 고유명사는 모델 특성상 변동해 베타 정확도로 기록했습니다.
...
| 실제 한국어 마이크 입력 | 마이크 버튼을 눌러 통제된 한국어 기준 음성을 입력하고 전사 결과를 확인 | 입력창 삽입 및 후속 AI/셸 실행 경로 확인 (완료, 고유명사 정확도는 베타 수준) |
...
- Whisper `base` 모델은 Metal CLI·무음 WAV 스모크와 실제 CPAL 캡처 후 전사 입력까지 정상 실행했다. 통제된 한국어 기준 음성은 문장 단위로 입력창에 삽입되며 `LUM` 고유명사 인식은 `늘럼/운량` 등으로 흔들릴 수 있다.
...
- 빠른 액션은 입력창 포커스 중에도 헤더의 `빠른 액션` 버튼으로 열 수 있고, 편집 다이얼로그의 빈 상태·입력 폼까지 Playwright 스모크로 고정했다.

`cargo fmt --check`는 릴리스 게이트 통과를 위해 rustfmt(비시맨틱)를 적용해 통과시켰다. (커밋 `9b58205`)
...
- `npm run build`: 성공
- `npx playwright test`: `30 passed`
- `npm run mvp:readiness`: 필수 9/9 PASS
